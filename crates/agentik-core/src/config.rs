...
impl Default for PermissionsConfig {
    fn default() -> Self {
        Self {
            default_allow: vec!["Read".to_string(), "Glob".to_string(), "Grep".to_string()],
            require_confirm: vec!["Write".to_string(), "Edit".to_string(), "Bash".to_string()],
            always_deny: vec![],
        }
    }
}
...
impl Config {
    ...
    pub fn validate(&self) -> ValidationResult {
        ...
        if self.limits.max_tokens > 200_000 {
            result.add_warning(
                "limits.max_tokens",
                "max_tokens is very high (> 200k), this may cause issues",
            );
        }
        ...
        if self.limits.max_context_files == 0 {
            result.add_error(
                ...
                "max_context_files must be greater than 0",
            );
        }
        ...
        if self.limits.cost_warning_threshold < 0.0 {
            result.add_error(
                "limits.cost_warning_threshold",
                "cost_warning_threshold cannot be negative",
            );
        }
        ...
        if !valid_color_modes.contains(&self.display.color.as_str()) {
            result.add_error(
                "display.color",
                format!(
                    "Invalid color mode '{}'. Valid values: {:?}",
                    self.display.color, valid_color_modes
                ),
            );
        }
        ...
        if !valid_sandbox_modes.contains(&self.sandbox.mode.as_str()) {
            result.add_error(
                "sandbox.mode",
                format!(
                    "Invalid sandbox mode '{}'. Valid values: {:?}",
                    self.sandbox.mode, valid_sandbox_modes
                ),
            );
        }
...
mod tests {
    ...
    fn test_invalid_max_tokens() {
        ...
        assert!(result
            .errors()
            .iter()
            .any(|e| e.field == "limits.max_tokens"));
    }
    ...
    fn test_negative_budget_is_error() {
        ...
        assert!(result
            .errors()
            .iter()
            .any(|e| e.field == "limits.daily_budget"));
    }
